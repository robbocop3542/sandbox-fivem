...
AddEventHandler("Core:Shared:Ready", function()
	exports["Paradise-base"]:RequestDependencies("Animations", {
		"Callbacks",
	}, function(error)
		if #error > 0 then
			return
...
function RegisterCallbacks()
	Callbacks:RegisterServerCallback("Paradise-dj:server:songInfo", function(source, data, cb)
		cb(previousSongs)
	end)

	Callbacks:RegisterServerCallback("Paradise-dj:server:playMusic", function(source, data, cb)
		local src = source
	...
	Callbacks:RegisterServerCallback("Paradise-dj:server:changeVolume", function(source, data, cb)
		local src = source
		local zoneNum = data.zoneNum
	...
	Callbacks:RegisterServerCallback("Paradise-dj:server:stopMusic", function(source, data, cb)
		local src = source
		...
		TriggerClientEvent("Paradise-dj:client:playMusic", src, { zone = data.zoneNum })
	end)

	Callbacks:RegisterServerCallback("Paradise-dj:server:pauseMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
		if Config.Locations[data.zoneNum].playing then
			Config.Locations[data.zoneNum].playing = nil
	...
	Callbacks:RegisterServerCallback("Paradise-dj:server:resumeMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
...
-- -- This is for that
-- RegisterNetEvent("Paradise-dj:server:DestoryAll", function()
...
-- 	TriggerEvent("Paradise-dj:server:DestoryAll")
-- end)
