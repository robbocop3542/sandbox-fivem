...
AddEventHandler("Core:Shared:Ready", function()
	exports["Paradise-base"]:RequestDependencies("DJ", {
		"Callbacks",
		"Targeting",
		"Menu",
		"Notification",
		...
		if v.enableBooth then
			Targeting.Zones:AddBox("dj-booth" .. k, "compact-disc", v.coords, 1.0, 1.0, {
			...
			}, {
				{
					icon = "compact-disc",
					text = "Open Stereo",
					event = "Paradise-dj:client:playMusic",
					data = { zone = k },
				},
			})
...
RegisterNetEvent("Paradise-dj:client:playMusic", function(data)
	local booth = ""
	local boothId = 0
	...
	local p = promise.new()
	Callbacks:ServerCallback("Paradise-dj:server:songInfo", {}, function(cb)
		p:resolve(cb)
	end)
	...
	djMenu.Add:Input("Youtube URL", {
		...
		Callbacks:ServerCallback("Paradise-dj:server:playMusic", {
			song = song,
			zoneNum = boothId,
		}, function(success, zoneNum)
			if success then
				TriggerEvent("Paradise-dj:client:playMusic", { zone = zoneNum })
	...
	if xSound:soundExists(booth) then
		if xSound:isPlaying(booth) then
			djMenu.Add:Button("Pause Music", { success = true }, function()
				Callbacks:ServerCallback("Paradise-dj:server:pauseMusic", {
					zoneName = booth,
					zoneNum = boothId,
				}, function(success)
					if success then
		...
		elseif xSound:isPaused(booth) then
			djMenu.Add:Button("Resume Music", { success = true }, function()
				Callbacks:ServerCallback("Paradise-dj:server:resumeMusic", {
					zoneName = booth,
					zoneNum = boothId,
				}, function(success)
					if success then
		...
		djMenuSub["changevolume"].Add:Slider("Change Volume", {
			...
			Callbacks:ServerCallback("Paradise-dj:server:changeVolume", {
				volume = volume,
				zoneName = booth,
				zoneNum = boothId,
			}, function(success, zoneNum)
		...
		djMenu.Add:Button("Stop Music", { success = true }, function()
			Callbacks:ServerCallback("Paradise-dj:server:stopMusic", {
				zoneName = booth,
				zoneNum = boothId,
			}, function(success)
				if success then
